...
class RandomTextlistforListbox:
    def random_number():
        # Generate random number
    ...
    def random_text_list(number):
        ...
        return text_list
def calculate_width_list(text):
    textlegth = len(text)
    list_of_words_length = []
    for oneworld in text:
        list_of_words_length.append(len(oneworld))

    max_width_value = max(list_of_words_length)
    random_width = random.randint(0, 120)
    width =0
    if (max_width_value *7+10) <random_width:
        width = random_width
    else:
        width = max_width_value *7+10
    return width
def calculate_height_list(text):
    textlegth = len(text)
    random_height = random.randint(0, 120)
    height =0
    if textlegth *16+10<random_height:
        height = random_height
    else:
        height = textlegth *16+10
    return height
...
def create_listboxes(num_rows, num_cols, screen_width, screen_height, list_of_element_positions, root,max_hight_value,max_width_value,horizental_distance,vertical_distance):
    number_for_list = RandomTextlistforListbox.random_number()
    # Use the class methods to generate random list of options for the dropdown menu
    listboxe_menu_options = RandomTextlistforListbox.random_text_list(number_for_list)
    # Use the class methods to generate random values for the dropdown menu configuration
    random_text = listboxe_menu_options
    width =calculate_width_list(random_text)
    height=calculate_height_list(random_text)

    selected_option = tk.StringVar(value=random_text)
    # Create a listbox
    listbox = tk.Listbox(root, listvariable=selected_option)
    # Place dropdown_menu on the window line by line
    if max_height_value < screen_height / vertical_distance * i and max_width_value < screen_width / horizental_distance * j:
        listbox.place(x=screen_width / horizental_distance * j, y=screen_height / vertical_distance * i,width=width,height=height )
    elif max_height_value > screen_height / vertical_distance * i and max_width_value < screen_width / horizental_distance * j:
        listbox.place(x=screen_width / horizental_distance * j, y=max_height_value,width=width,height=height)
    elif max_height_value < screen_height / vertical_distance * i and max_width_value > screen_width / horizental_distance * j:
        listbox.place(x=max_width_value, y=screen_height / vertical_distance * i,width=width,height=height)
    else:
        listbox.place(x=max_width_value, y=max_height_value,width=width,height=height)
    # Set checkbutton size randomly
    # Configure the font of the Checkbutton widget using random_font() function
    # config the font of the dropdown menu
    ...
    time.sleep(0.1)
    # Get checkbutton position coordinates
    widget_x1, widget_y1 = listbox.winfo_rootx(), listbox.winfo_rooty()
    widget_x2, widget_y2 = widget_x1 + listbox.winfo_width(), widget_y1 + listbox.winfo_height()
    listbox_position = ("listbox", (widget_x1, widget_x2), (widget_y1, widget_y2), random_text, "Checked ",
                            ("font[0]", "font[1]", "font[2]", "font[3])"))
    # print("scrollbar " + str(i) + "_" + str(j), scrollbar_orientation, scrollbar_height, scrollbar_width)
    list_of_element_positions.append(listbox_position)
    # Return the list of checkbutton positions
    return list_of_element_positions
    ...
    for i in range(Rows_number):
        ...
        for j in range(Columns_number):
            ...
            if x == 0:
                ...
                create_listboxes(i, j, screen_width, screen_height, list_of_Element_position, root,max_height_value,max_width_value,disatnince_between_elements_vertical,disatnince_between_elements_horizontal)
